...
const CarrierForm: React.FC<CarrierFormProps> = ({ mode }) => {
  ...
  const [carrier, setCarrier] = useState<Partner>({
    id: undefined,
    name: '',
    type: 'carrier', // Default to carrier
    legal_form: '',
    registration_number: '',
    vat_number: '',
    website: '',
  ...
  const validateForm = (): boolean => {
    ...
    // Type is fixed to 'carrier' for this form, so no validation needed for it
    
    if (carrier.website && !isValidUrl(carrier.website)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(null);
      if (isEditMode) {
        await updatePartner(id!, carrier);
        navigate(`/carriers/${id}`);
      } else {
        await createPartner(carrier);
        navigate('/carriers');
      }
    } catch (err: any) {
      setError(err.message || 'Une erreur est survenue lors de l\'enregistrement du transporteur');
      setSaving(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-4 py-5 sm:p-6">
            <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              {/* Type (hidden or disabled as it's fixed to 'carrier') */}
              <div className="sm:col-span-2">
                ...
                <div className="mt-1">
                  <input
                    ...
                    value="Transporteur"
                    disabled
                    className="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md bg-gray-50 cursor-not-allowed"
                  />
                </div>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="mt-1">
                  <input
                    ...
                    value={carrier.registration_number || ''}
                    onChange={handleChange}
                    className="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md"
                  />
                </div>
              ...
              <div className="sm:col-span-6">
                ...
                <p className="mt-2 text-sm text-gray-500">
                  Informations complémentaires sur le transporteur (capacités spéciales, zones de couverture, certifications, etc.).
                </p>
              </div>
            </div>
          </div>
